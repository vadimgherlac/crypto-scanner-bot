...
SIGNALS_FILE    = "trade_signals_v14.csv"
DAILY_LOCK_FILE = "daily_risk_lock_v14.csv"
PAIR_STATS_FILE = "pair_stats_v14.csv"
...
def is_hammer(df: pd.DataFrame) -> bool:
    """Bullish reversal pin bar: small body, long lower wick."""
    latest = df.iloc[-1]
...
def is_shooting_star(df: pd.DataFrame) -> bool:
    """Bearish reversal pin bar: small body, long upper wick."""
    latest = df.iloc[-1]
    body   = abs(latest["close"] - latest["open"])
    rng    = latest["high"] - latest["low"]
    if rng <= 0:
...
def is_inside_bar_breakout_long(df: pd.DataFrame) -> bool:
    """Inside bar with bullish breakout: current bar exceeds mother bar high."""
    if len(df) < 3:
        return False
    mother  = df.iloc[-3]
    inside  = df.iloc[-2]
    current = df.iloc[-1]
    inside_bar = inside["high"] < mother["high"] and inside["low"] > mother["low"]
    return inside_bar and current["close"] > mother["high"]
...
def is_inside_bar_breakout_short(df: pd.DataFrame) -> bool:
    ...
    current = df.iloc[-1]
    inside_bar = inside["high"] < mother["high"] and inside["low"] > mother["low"]
    return inside_bar and current["close"] < mother["low"]

# =========================================================
# NEW: VOLUME PROFILE (approximate)
# Find the price level with the most traded volume (POC)
# Check if current price is near POC (within 0.5 * ATR)
# =========================================================
def compute_poc(df: pd.DataFrame, bins: int = 20) -> float:
    """Return the price level (POC) with highest volume in recent data."""
    if len(df) < 20:
...
def near_poc(price: float, poc: float, atr: float) -> bool:
    """Price is within 0.5 ATR of the POC — high-value area."""
    return abs(price - poc) <= (atr * 0.5)

# =========================================================
# NEW: SMARTER STOP PLACEMENT
# Stop behind the nearest swing high/low instead of flat ATR
# =========================================================
def smart_stop_long(df: pd.DataFrame, atr: float, lookback: int = 10) -> float:
    """Place stop just below the most recent swing low, with ATR buffer."""
...
def smart_stop_short(df: pd.DataFrame, atr: float, lookback: int = 10) -> float:
    """Place stop just above the most recent swing high, with ATR buffer."""
    recent_high = df["high"].iloc[-(lookback+2):-1].max()
    return round(recent_high + (0.25 * atr), 4)

# =========================================================
# EXISTING PRICE ACTION LOGIC
# =========================================================
...
def is_daily_locked():
    ...
    if lock["loss_count"] >= MAX_LOSSES_PER_DAY:
        return True, f"Max losses reached ({lock['loss_count']})"
    if lock["daily_risk_used"] >= max_risk:
        return True, f"Max daily risk reached (${lock['daily_risk_used']:.2f})"
...
def update_signal_results():
    ...
    for i, row in df.iterrows():
        ...
        side        = row["side"]
        stop        = float(row["stop"])
        tp2         = float(row["tp2"])          # use TP2 as final target
        ...
        try:
            if asset_type == "crypto":
                ...
                for idx, c in candles.iterrows():
                    h = float(to_series(c["High"]) if isinstance(c["High"], pd.Series) else c["High"])
                    l = float(to_series(c["Low"])  if isinstance(c["Low"],  pd.Series) else c["Low"])
                    if side == "BUY":
                        if l <= stop:
...
def build_diagnostic_report(scan_log: list) -> str:
    ...
    lines = ["🔍 V14 DIAGNOSTIC — Top Candidates\n"]
    for e in best:
        bl = ", ".join(e["blockers_long"])  or "✅ clear"
        bs = ", ".join(e["blockers_short"]) or "✅ clear"
        lines.append(
            f"{e['symbol']}\n"
            f"  Long  {e['long_score']:>3} | blocked: {bl}\n"
            f"  Short {e['short_score']:>3} | blocked: {bs}"
        )
    return "\n".join(lines)

# =========================================================
# CORE SIGNAL BUILDER
# Shared logic for both crypto and stocks
# =========================================================
def build_signal(
    symbol: str,
    asset_type: str,
    df_5m: pd.DataFrame,   # standardised lowercase cols
    df_15m: pd.DataFrame,
    df_1h: pd.DataFrame,
    df_4h: pd.DataFrame,
    ...
    adx_ok    = adx_val >= 20              # trend is strong enough
    ...
    # Thresholds scaled to new score range
    min_score = 9 if session_strength == "HIGH" else 10 if session_strength == "MID" else 11
    ...
    buy_signal = (
        ...
        not choppy and adx_ok and
        trend_4h_bull and trend_1h_bull and trend_15m_bull and
    ...
    sell_signal = (
        ...
        not choppy and adx_ok and
        trend_4h_bear and trend_1h_bear and trend_15m_bear and
    ...
    if not adx_ok:                blockers_long.append(f"ADX={adx_val:.1f}<20")
    if not trend_4h_bull:         blockers_long.append("4H bear")
    ...
    if not adx_ok:                blockers_short.append(f"ADX={adx_val:.1f}<20")
    if not trend_4h_bear:         blockers_short.append("4H bull")
    ...
    if buy_signal:
        ...
        tp1    = round(entry + (1.5 * abs(entry - stop)), 4)
        tp2    = round(entry + (2.5 * abs(entry - stop)), 4)
        ...
        if key not in sent_alerts:
            ...
            msg = (
                ...
                f"ADX: {adx_val:.1f} | Score: {long_score}\n"
                f"PA: {pa_str}\n"
                f"Candles: {can_str}\n"
                f"Div: bull={bull_div} | POC: {poc:.4f} near={near_value}"
                + (f"\n\n🚀 BYBIT:\nTRADE {symbol} BUY {qty} {stop} {tp2}" if asset_type=="crypto" else "")
            )
            send_telegram_message(msg)
            log_signal(timestamp, symbol, asset_type, "BUY", "5m/15m/1h/4h",
                       entry, stop, tp1, tp2, round(rsi,2), round(vwap,4),
                       round(atr,4), round(adx_val,1), long_score, long_grade,
                       session_strength, rr, f"V14 long: {pa_str} {can_str}")
            print(f"✅ {long_grade} BUY sent: {symbol}")

    elif sell_signal:
        entry  = price
        stop   = smart_stop_short(df_5m, atr)
        tp1    = round(entry - (1.5 * abs(stop - entry)), 4)
        tp2    = round(entry - (2.5 * abs(stop - entry)), 4)
        rr     = calculate_rr(entry, stop, tp2)
        qty    = get_crypto_qty(symbol, entry, stop) if asset_type == "crypto" else "N/A"
        ...
        if key not in sent_alerts:
            ...
            msg = (
                ...
                f"Qty:    {qty}\n"
                f"RSI: {rsi:.1f} | VWAP: {vwap:.4f} | ATR: {atr:.4f}\n"
                f"ADX: {adx_val:.1f} | Score: {short_score}\n"
                f"PA: {pa_str}\n"
                f"Candles: {can_str}\n"
                f"Div: bear={bear_div} | POC: {poc:.4f} near={near_value}"
                + (f"\n\n🚀 BYBIT:\nTRADE {symbol} SELL {qty} {stop} {tp2}" if asset_type=="crypto" else "")
            )
            send_telegram_message(msg)
            log_signal(timestamp, symbol, asset_type, "SELL", "5m/15m/1h/4h",
                       entry, stop, tp1, tp2, round(rsi,2), round(vwap,4),
                       round(atr,4), round(adx_val,1), short_score, short_grade,
                       session_strength, rr, f"V14 short: {pa_str} {can_str}")
            print(f"✅ {short_grade} SELL sent: {symbol}")

    else:
        print(f"{symbol}: no signal | L={long_score} S={short_score} | "
              f"choppy={choppy} adx={adx_val:.1f} pa_l={pa_long_ok} pa_s={pa_short_ok}")
...
def scan_stock(ticker: str, scan_log: list):
    ...
    try:
        ...
        df_raw_4h  = get_data(ticker, "1h",  "60d")   # yfinance max 1h; we resample to 4h

        if any(d.empty for d in [df_raw_5m, df_raw_15m, df_raw_1h, df_raw_4h]):
        ...
        # Resample 1h data to 4h for stocks
        df_raw_4h_rs = df_raw_1h.resample("4h").agg({
            "Open": "first", "High": "max", "Low": "min",
            "Close": "last", "Volume": "sum"
        }).dropna()
        df_4h = yf_to_std(df_raw_4h_rs)

        for df in [df_5m, df_15m, df_1h, df_4h]:
            if len(df) < 60:
...
def main():
    ...
    send_telegram_message("✅ Scanner bot V14 started\n4H filter | ADX | RSI div | Candles | POC | Smart stops | Dual TP")
    print("Bot V14 started.")
